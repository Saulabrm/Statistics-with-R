...
ggplot(data =subset(nc, !is.na(habit)), aes(y=weight,x= habit, fill=habit )) + geom_boxplot() + theme_bw()
```
...
nc %>%
  group_by(habit) %>%
  summarise(mean_weight = mean(weight), total = n())
...
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ci", 
          conf_level = .95, method = "theoretical")
```
...
inference(y = weeks, data = nc, statistic = "mean", type = "ci", 
          conf_level = .90, method = "theoretical")
```
...
nc %>% group_by(mature) %>% summarize("min_age" = min(mage),
                                      "max_age" = max(mage))
```
