...
samp1 %>%
  summarise(xbar = mean(area), pop_med = median(area), 
            stddv = sd(area), pop_iqr = IQR(area),
            pop_min = min(area), pop_max = max(area),
            pop_q1 = quantile(area, 0.25),  # first quartile, 25th percentile
            pop_q3 = quantile(area, 0.75))  # third quartile, 75th percentile
```
...
ames %>% sample_n(size=50) %>% summarise(xbar = mean(price))
...
samples_price = ames %>% rep_sample_n(size=50,reps = 5000, replace = TRUE) %>% summarise(xbar = mean(price))

ggplot(samples_price, aes(x=xbar)) + geom_histogram()
```
...
# type your code for this Exercise here, and Run Document
sample_means150 = ames %>% rep_sample_n(size=150,reps = 5000, replace = TRUE) %>% summarise(xbar = mean(price))

ggplot(sample_means150, aes(x=xbar)) + geom_histogram()
```
...
# type your code for this Exercise here, and Run Document
ames %>% sample_n(size=15,replace=TRUE) %>% summarize(xbar=mean(price))
```
...
# type your code for this Exercise here, and Run Document
sample_means15 = ames %>% rep_sample_n(size=15,replace=TRUE, reps=2000) %>% summarise(xbar=mean(price))

"sampling" 
mean(sample_means15$xbar)
