...
```{r set-seed}
set.seed(000001)                 # make sure to change the seed
```
...
ggplot(ames, aes(x=area)) + geom_histogram()
```
...
<ol>
<li> 95% of the time the true average area of houses in Ames, Iowa, will be in this interval.tp </li>
...
if(findInterval(params,interval)>0){print("TRUE")}else{print("FALSE")}
```
...
92%. It is lower than our confidence interval estated.
...
ci = ames %>% rep_sample_n(size =n,reps=50, replace=T) %>%
               summarise( lower = mean(area) - z_star_99 * (sd(area)/sqrt(n)),
                          upper = mean(area) + z_star_99* (sd(area)/sqrt(n)))

ci = ci %>% mutate(capture_mu = ifelse(lower< params$mu & upper > params$mu,
                                  "yes",
                                  "no"))
