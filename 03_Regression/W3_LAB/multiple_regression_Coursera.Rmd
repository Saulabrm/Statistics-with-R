...
```{r score-dist}
# Checking median, quartiles, scores below 3 and plotting.


summary(evals$score)

evals %>% select(score) %>% arrange(score) %>% filter(score < 3)

ggplot(evals,aes(x=score)) + geom_histogram(fill="blue", col="black", bins = 30) + theme_classic() 
...
ggplot(evals,aes(cls_perc_eval)) + geom_histogram(aes(fill=gender), position = "stack") + theme_classic()
```
...
p1<-ggplot(m_bty, aes(.fitted, .resid))+geom_point()
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_no_prf <- lm(score ~ rank + ethnicity + gender + 
                      language + age + cls_perc_eval 
                       + cls_students + cls_level + cls_credits + bty_avg 
                       + pic_outfit + pic_color, data = evals)
summary(m_full_no_prf)
```
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
m3_bty_avg = lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_profs + cls_credits, data = evals)
summary(m3_bty_avg)$adj.r.squared

m4_cls_profs= lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + 
    cls_students + cls_level + cls_credits + bty_avg , data = evals)
summary(m4_cls_profs)$adj.r.squared

m5_cls_students= lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval + 
    cls_profs + cls_level + cls_credits + bty_avg , data = evals)
summary(m5_cls_students)$adj.r.squared
