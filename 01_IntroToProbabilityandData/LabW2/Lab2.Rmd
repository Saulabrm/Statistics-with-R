...
ggplot(sfo_feb_flights, aes(x=arr_delay)) + geom_histogram(binwidth = 10)
```
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(med = median(arr_delay), iqr = IQR(arr_delay))
```
...
nycflights %>%
  ...
  arrange(desc(median_dd)) %>% slice(1)
...
nycflights %>%
  group_by(origin) %>%
  summarise(ot_dep_rate = sum(dep_type == "on time") / n()) %>%
  arrange(desc(ot_dep_rate)) %>% slice(1)
```
...
nycflights %>% mutate(avg_speed = distance/(air_time/60)) %>% arrange(desc(avg_speed)) %>% select(avg_speed, tailnum) %>% slice(c(1,2))
...
nycflights %>%
  ...
  filter(dep_type=="delayed") %>% summarise(ot_ar_rate = sum(arr_type=="on time")/n())
```